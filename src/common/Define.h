...
const uint8 ACCOUNT_NAME_SIZE			= TINYSTR;
const uint8 PASSWORD_SIZE				= TINYSTR;
const uint8 PLAYER_NAME_SIZE			= TINYSTR; 
const uint8 PLAYER_SIGN_SIZE			= TINYSTR;
const uint8 EMAIL_SIZE					= 64;
const uint8	MAX_OFFICER_NUM				= 64;
...
const uint8 MAX_PSKILL_COUNT			= 6;						//最大被动技能数量
...
const uint8 MAX_PSKILL_UNLOCK_COUNT		= 6;						//被动技能解锁数量
...
const uint32 OFFICER_PROTECT_CD			= 24 * 3600;				// 晋升/降级被挑战的保护时间
const uint32 OFFICER_FAIL_CD			= 2 * 3600;					// 挑战失败的CD时间
const uint32 OFFICER_DEMOTE_CD			= 3 * 24 * 3600;			// 挑战失败的CD时间

const uint8	MAX_OFFICER_PLAYR_LIST		= 4;						// 每次刷出的不同模式下，最大的玩家列表数目

const int SERVER_GATEWAY				= 1;
#define GAME_NOVICE_CARD "NCH"										// 新手卡激活码前3位标示

#define OFFICER_RESORT_WEEKDAY	"2,4,6"								// 爵位降职的星期数(1~7)
...
enum PlayerDataType
{
	...
	PlayerDataType_Mart			= (0x00000001 << 23),           //显示购买数据
    PlayerDataType_WorldBoss    = (0x00000001 << 24),           //世界Boss
};

enum SystemDataType
{
    SystemDataType_Init       = (1 << 0), // 系统初始化
    SystemDataType_Mail       = (1 << 1), // 系统邮件
    SystemDataType_Boss       = (1 << 2), // 世界Boss
    SystemDataType_Mine       = (1 << 3), // 抢矿
